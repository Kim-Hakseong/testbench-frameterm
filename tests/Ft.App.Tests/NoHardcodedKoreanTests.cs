...
/// <summary>
/// 이 앱의 기본 사용자 언어는 영어다. 자매 앱(Modbus Workbench)에서 36개의
/// 한글 문자열이 코드에 박힌 채 영어 UI에 렌더링되고 있었고, 테스트는 전부
/// 통과하고 있었다 — 화면을 눈으로 보기 전에는 드러나지 않는 종류였다.
/// 같은 일이 여기서 벌어지지 않도록 소스를 직접 훑는다.
///
/// **주석은 대상이 아니다** — 이 저장소의 주석은 한국어로 쓴다. 문제는
/// 사용자에게 보이는 문자열 리터럴이다.
/// </summary>
public sealed class NoHardcodedKoreanTests
{
    ...
    /// <summary>
    /// 검사 제외 경로. FrameTerm은 도움말을 한/영 두 벌로 싣고 언어 토글로
    /// 전환하므로, 그 두 곳의 한국어는 버그가 아니라 콘텐츠다.
    /// </summary>
    private static readonly string[] Allowed =
    [
        Path.Combine("Ft.App", "Services", "HelpContent.cs"),
        Path.Combine("Ft.App", "Views", "HelpWindow.axaml"),
        Path.Combine("Ft.App", "Views", "MainWindow.axaml"),
    ];
